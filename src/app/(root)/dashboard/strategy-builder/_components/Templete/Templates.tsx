...
const TemplateCard: React.FC<TemplateCardProps> = ({ name, description, onSelect }) => (
  <div 
    onClick={onSelect}
  ...
  <Dialog open={isOpen} onOpenChange={onClose}>
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          Are you sure you want to apply the {templateName} template? This will replace your current canvas.
        </DialogDescription>
      </DialogHeader>
      <DialogFooter>
  ...
  return (
    <>
      <Dialog>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            {(Object.entries(STRATEGY_TEMPLATES) as [TemplateKey, typeof STRATEGY_TEMPLATES[TemplateKey]][]).map(([key, template]) => (
              <TemplateCard
                key={key}
                name={template.name}
      ...
      <ConfirmDialog
        ...
        templateName={selectedTemplate ? STRATEGY_TEMPLATES[selectedTemplate].name : ''}
      />
    </>
  );
};